	...
	if(kboard_mod == 'editor'){
		...
		if(kboard_current.use_editor == 'snote'){ // summernote
			jQuery('.summernote').each(function(){
				...
				summernote.closest('form').off('submit.kboardTableWidths').on('submit.kboardTableWidths', function(){
					jQuery('.summernote', this).each(function(){
						var field = jQuery(this);
						var editable = field.next('.note-editor').find('.note-editable');
						if(editable.length){
							kboard_editor_normalize_table_widths(editable);
							field.val(editable.html());
						}
					});
				});
			});
...
/**
 * Normalize percentage widths generated by the browser's native table resizer.
 * Some browsers can leave widths such as 48% and 368% on cells in the same row.
 * The browser renders those values as a ratio, so preserve that ratio while
 * making the row total 100% before Summernote serializes the content.
 */
function kboard_editor_normalize_table_widths(editable){
	jQuery('table tr', editable).each(function(){
		var cells = jQuery(this).children('th, td');
		var widths = [];
		var total = 0;
		var can_normalize = cells.length > 1;

		cells.each(function(){
			var cell = jQuery(this);
			var style_width = this.style ? this.style.width : '';
			var attr_width = cell.attr('width') || '';
			var width = style_width || attr_width;
			var match = /^\s*(\d+(?:\.\d+)?)%\s*$/.exec(width);
		...
		if(!can_normalize || !total || Math.abs(total - 100) < 0.01){
			return;
		}
		...
		cells.each(function(index){
			var cell = jQuery(this);
			var normalized = index === cells.length - 1 ? 100 - assigned : Math.round((widths[index] / total * 100) * 10000) / 10000;
			var value = normalized.toFixed(4).replace(/\.?0+$/, '') + '%';

			assigned += normalized;
			this.style.width = value;
			if(cell.attr('width') !== undefined){
